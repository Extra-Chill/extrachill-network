...
Lifecycle option writes acquire a network-scoped MySQL advisory lock with `GET_LOCK(..., 0)` before reading the option. After acquisition, Network deletes Core's exact `network_id:extrachill_experiment_lifecycle` key from the `site-options` cache and clears a matching `notoptions` marker before reading. This prevents a request-local preloaded value from overwriting a transition committed by the previous lock holder. The complete fresh read/modify/write occurs while held, Core restores cache coherence during update, and `RELEASE_LOCK()` is guaranteed in `finally`. Acquisition, cache refresh, or release errors fail closed.
